 ...
 * @author sameh
 */
public class AddToClassVisitor implements OWLObjectVisitor {
    
    private OClass clss;
    
    public AddToClassVisitor(OClass clss) {
        this.clss = (OClass)clss;
    }
    
    @Override
    public void visit(OWLClass owlc) {
        ID id = new ID(owlc.getIRI().toURI());
        clss = Memory.get(id, OClass.class);
    }
